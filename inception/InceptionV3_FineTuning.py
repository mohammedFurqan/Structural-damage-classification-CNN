...
#model = load_model('InceptionV3_finetuning.h5')
#-----------------------------------------
...
with open('hist_InceptionV3_fine_tuning.pickle', 'wb') as handle:
...
for i in range(batch_size):
    namepath = 'InceptionV3_finetuning/output_0/' + str(i) + '.png'
    matplotlib.image.imsave(namepath, test_X[i])
#--------------------------------------------------------------
#

# print intermediate feature
layer_name1 = base_model.layers[1].name
intermediate_layer_model1 = Model(inputs=model.input,
                                 outputs=model.get_layer(layer_name1).output)
intermediate_output1 = intermediate_layer_model1.predict(test_X[0:batch_size,:,:,:])
...
for i in range(n):
    ...
    for j in range(m):
        namepath = 'InceptionV3_finetuning/output_1/' + str(i) + '_'+ str(j) + '.png'
        matplotlib.image.imsave(namepath, intermediate_output1[i,:,:,j])

#-----------------------------------------------------
layer_name2 = base_model.layers[4].name
intermediate_layer_model2 = Model(inputs=model.input,
                                 outputs=model.get_layer(layer_name2).output)
intermediate_output2 = intermediate_layer_model2.predict(test_X[0:batch_size,:,:,:])
print(intermediate_output2.shape)

for i in range(n):
    m = intermediate_output2.shape[3]
    for j in range(m):
        namepath = 'InceptionV3_finetuning/output_2/' + str(i) + '_'+ str(j) + '.png'
        matplotlib.image.imsave(namepath, intermediate_output2[i,:,:,j])

#-----------------------------------------------------------------------
layer_name3 = base_model.layers[7].name
intermediate_layer_model3 = Model(inputs=model.input,
                                 outputs=model.get_layer(layer_name3).output)
intermediate_output3 = intermediate_layer_model3.predict(test_X[0:batch_size,:,:,:])
print(intermediate_output3.shape)

for i in range(n):
    m = intermediate_output3.shape[3]
    for j in range(m):
        namepath = 'InceptionV3_finetuning/output_3/' + str(i) + '_'+ str(j) + '.png'
        matplotlib.image.imsave(namepath, intermediate_output3[i,:,:,j])

#----------------------------------------------------------------------
layer_name4 = base_model.layers[11].name
intermediate_layer_model4 = Model(inputs=model.input,
                                 outputs=model.get_layer(layer_name4).output)
intermediate_output4 = intermediate_layer_model4.predict(test_X[0:batch_size,:,:,:])
print(intermediate_output4.shape)

for i in range(n):
    m = intermediate_output4.shape[3]
    for j in range(m):
        namepath = 'InceptionV3_finetuning/output_4/' + str(i) + '_'+ str(j) + '.png'
        matplotlib.image.imsave(namepath, intermediate_output4[i,:,:,j])

#---------------------------------------------------------------------
layer_name5 = base_model.layers[14].name
intermediate_layer_model5 = Model(inputs=model.input,
                                 outputs=model.get_layer(layer_name5).output)
intermediate_output5 = intermediate_layer_model5.predict(test_X[0:batch_size,:,:,:])
print(intermediate_output5.shape)

for i in range(n):
    m = intermediate_output5.shape[3]
    for j in range(m):
        namepath = 'InceptionV3_finetuning/output_5/' + str(i) + '_'+ str(j) + '.png'
        matplotlib.image.imsave(namepath, intermediate_output5[i,:,:,j])
